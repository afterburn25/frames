...
def one(old,new,label):
    global src
    n=src.count(old)
    if n!=1: raise SystemExit(f'r59n cert anchor {label} count {n}')
    src=src.replace(old,new,1)
def alln(old,new,count,label):
    global src
    n=src.count(old)
    if n!=count: raise SystemExit(f'r59n cert anchor {label} count {n}, expected {count}')
    src=src.replace(old,new)

one("'patch_v108_r59m_hub_multi_tt_activation.py'","'patch_v108_r59n_periodic_window_forensics.py'",'patch target')
alln('kernel-r59m.nx','kernel-r59n.nx',2,'kernel evidence target')
one('8b236b8b21a181e5db9fbeec3c5b64840df0d3158980bde3176647e6cf651bc8','eff364295a51eae11757d39d05f406934ebfe16be84e733ee5a2120e3635de08','exact r59n identity target')
...
try:
    ...
    for q in ('volatile_read64(xhci+3880)','volatile_read64(xhci+3888)','fls=(c/4)%4','fi=(fr/8)%1024','volatile_read32(frame+(fi*4))==qlo+2','volatile_read32(dm+12)==tdlo'):
        if q not in s: raise SystemExit('r59n inherited r59m/r59l evidence witness missing '+q)
    low=tick.lower()
