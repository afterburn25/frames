...
def main():
    ...
    result={'status':'FAIL','lane':a.lane,'iso_sha256':sha256(a.iso)}
    ...
    try:
        ...
        for _ in range(800):
            ...
            time.sleep(.1)
        else: raise RuntimeError('stable runtime marker timeout')
        def capture(name):
        ...
        if pos0 is None: raise RuntimeError('initial cursor not found after stable marker')
        ...
        time.sleep(.15); after=capture('AFTER')
        pos1=locate_cursor(after,pos0[0]+8,pos0[1]+4,120)
        if pos1 is None: raise RuntimeError('moved cursor not found')
        changed,bbox=frame_delta(before,after); dx=pos1[0]-pos0[0]; dy=pos1[1]-pos0[1]
        ghost0=before.getpixel((291,17))!=WHITE; ghost1=after.getpixel((291,17))!=WHITE
        result.update(markers=True,initial_cursor=list(pos0),final_cursor=list(pos1),dx=dx,dy=dy,changed_pixels_outside_overlay=changed,changed_bbox=bbox,legacy_ghost_absent=ghost0,legacy_ghost_after_absent=ghost1)
        ...
        if not (ghost0 and ghost1): raise RuntimeError('legacy ghost cursor detected')
