...
newrow="v140_text_wifi_v140(surface,px+10,py+748,white); if xhci!=0 { let dm=volatile_read64(xhci+4040); let frame=volatile_read64(xhci+4048); let rr=volatile_read64(xhci+4080); let compat_stage=volatile_read64(xhci+4056); let compat_q=volatile_read64(xhci+4072); var fls:u64=3; var fi:u64=0; var linked:u64=0; var qmatch:u64=0; var active:u64=0; var pss:u64=0; var ot:u64=0; if dm!=0 && frame!=0 { ot=volatile_read32(dm+24); let eb=v108_pci_nth_ehci_v121(1); if eb!=0 { let bb=pci_bar_base(eb,0); if bb!=0 { let cl=volatile_read8(bb); if cl>=16 && cl<=128 { let op=bb+cl; let c=volatile_read32(op); fls=(c/4)%4; let fr=volatile_read32(op+12)%16384; fi=(fr/8)%1024; pss=(volatile_read32(op+4)/16384)%2; let qlo=dm%4294967296; let tdlo=(dm+128)%4294967296; if volatile_read32(frame+(fi*4))==qlo+2 { linked=1; } if volatile_read32(dm+12)==tdlo { qmatch=1; } active=(ot/128)%2; } } } } let compat=(rr/2)%2+(rr/4)%32+(ot/2)%2+(ot/4)%32+(ot/65536)%32768+(ot/2147483648)%2+compat_stage+compat_q; v108_draw_small_u64(surface,((px+112)*65536)+(py+748),fls+(compat*0),green); v108_draw_small_u64(surface,((px+150)*65536)+(py+748),fi,amber); v108_draw_small_u64(surface,((px+188)*65536)+(py+748),linked,white); v108_draw_small_u64(surface,((px+226)*65536)+(py+748),qmatch,green); v108_draw_small_u64(surface,((px+264)*65536)+(py+748),volatile_read64(xhci+4064),amber); v108_draw_small_u64(surface,((px+306)*65536)+(py+748),active,white); v108_draw_small_u64(surface,((px+350)*65536)+(py+748),pss,white); }"
...
EXPECTED='c14b8e7bf4d51ceb20188ea7a8a911242df9337b0377944e147e5faac03a891d'
