...
def main():
    ...
    result={'status':'FAIL','gate':'r23_dragright_guard','iso_sha256':sha256(a.iso),'runtime_ready':False,'hover_no_repaint':False,'focus_transfer':False,'right_direct':False,'right_direct_marker_observed':False,'right_local_no_repaint':False,'dismiss_no_repaint':False,'repeat_no_repaint':False,'drag_motion_marker':False,'drag_live':False,'drag_commit':False,'drag_no_repaint':False,'left_right_collision_guard':False,'context_singleton':False}
    ...
    try:
        ...
        drag_before_ppm=out/'R23-DRAG-BEFORE.ppm'; call('screendump',{'filename':str(drag_before_ppm)})
        ...
        ctx_before_collision=text().count('FRAMES_V108_DESKTOP_CONTEXT_OK')
        for i in range(10):
            rel(12,8); time.sleep(.035)
            if i==3:
                btn('right',True); time.sleep(.006); btn('right',False); time.sleep(.02)
        drag_mid_ppm=out/'R23-DRAG-MID.ppm'; call('screendump',{'filename':str(drag_mid_ppm)})
        ...
        result['left_right_collision_guard']=mid.count('FRAMES_V108_DESKTOP_CONTEXT_OK')==ctx_before_collision
        ...
        if not result['left_right_collision_guard']: raise RuntimeError('transient right edge stole active left drag or opened context menu')
        ...
        size_ppm=out/'R23-SIZE.ppm'; call('screendump',{'filename':str(size_ppm)})
        ...
        singleton_ctx=t.count('FRAMES_V108_DESKTOP_CONTEXT_OK'); singleton_rep=t.count('FRAMES_V108_FULL_REPAINT_V120')
        btn('right',True); time.sleep(.012); btn('right',False); rel(3,2); rel(-2,1); time.sleep(.18); t=text()
        result['context_singleton']=t.count('FRAMES_V108_DESKTOP_CONTEXT_OK')==singleton_ctx and t.count('FRAMES_V108_FULL_REPAINT_V120')==singleton_rep
        if not result['context_singleton']: raise RuntimeError('active context menu accepted a duplicate right-click or repainted')
        ...
        err.close(); (out/'R23-INTERACTION.json').write_text(json.dumps(result,indent=2)+'\n')
        try:q.unlink()
