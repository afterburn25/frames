...
def main():
    ...
    cmd=['qemu-system-x86_64','-machine','q35','-m','768M','-smp','2','-cpu','max','-accel','tcg,thread=single','-display','none','-no-reboot','-no-shutdown','-nic','none','-serial',f'file:{serial}','-qmp',f'unix:{qmp},server=on,wait=off','-drive',f'if=pflash,format=raw,readonly=on,file={a.ovmf}','-cdrom',a.iso,'-boot','d','-device','qemu-xhci,id=xhci']
    if a.topology=='direct':
        cmd += ['-device','usb-mouse,bus=xhci.0,port=1,id=usbmouse']
    else:
        cmd += ['-device','usb-hub,bus=xhci.0,port=1,id=hub','-device','usb-mouse,bus=xhci.0,port=1.1,id=usbmouse']
    (out/'qemu-command.json').write_text(json.dumps(cmd,indent=2)+'\n')
    p=subprocess.Popen(cmd,stdout=subprocess.DEVNULL,stderr=stderr)
    result={'status':'FAIL','gate':'v108_usb_hub_topology_r3','topology':a.topology,'iso_sha256':iso_sha,'runtime_ready':False,'hub_found':False,'hub_child_found':False,'hub_child_hid':False,'usb_live_report':False,'usb_gui_cursor':False}
    try:
        ...
        for _ in range(120):
            ...
            result['hub_child_found']='FRAMES_USB_HUB_CHILD_FOUND' in txt
            ...
            if result['usb_live_report'] and result['usb_gui_cursor']:
                if a.topology=='direct' or (result['hub_found'] and result['hub_child_found'] and result['hub_child_hid']): break
        if not (result['usb_live_report'] and result['usb_gui_cursor']): raise RuntimeError('live USB report/cursor markers not reached')
        if a.topology=='hub' and not (result['hub_found'] and result['hub_child_found'] and result['hub_child_hid']): raise RuntimeError('hub discovery/child HID markers incomplete')
        result['status']='PASS'
        try: call('quit')
        ...
        stderr.close(); (out/'TOPOLOGY.json').write_text(json.dumps(result,indent=2)+'\n')
        try:qmp.unlink()
        except Exception:pass
