...
fn flight_fat32_find_root_v125(msc:u64,pstart:u64,data_start:u64,fat_start:u64,spc:u64,root:u64,which:u64,tagbase:u64) -> u64 {
...
fn flight_fat32_contig_v125(msc:u64,fat_start:u64,start:u64,clusters:u64,tagbase:u64) -> u64 {
    if start<2 || clusters==0 { return 0; } var c=start; var left=clusters; var group:u64=0;
    ...
    if left!=1 { return 0; } let end=flight_fat_next_v125(msc,fat_start,c,tagbase+256); if end<268435448 { return 0; } return 1;
}
'''
idx=s.index('fn flight_log_arm_v125')
s=s[:idx]+fat_helpers+s[idx:]
...
new_arm=r'''fn flight_log_arm_v125(fr:u64,msc:u64) -> u64 {
    ...
    let fat_start=pstart+reserved; let data_start=pstart+reserved+(nf*fatsz);
    let loginfo=flight_fat32_find_root_v125(msc,pstart,data_start,fat_start,spc,root,1,300); let taginfo=flight_fat32_find_root_v125(msc,pstart,data_start,fat_start,spc,root,2,1600);
    ...
    let cluster_bytes=spc*512; let needed=(logsize+cluster_bytes-1)/cluster_bytes; if flight_fat32_contig_v125(msc,fat_start,logcluster,needed,2500)==0 { flight_record_v125(fr,262401,8,logcluster); return 0; }
